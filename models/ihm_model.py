...
try:
    ...
    raise ImportError("Incorrect TensorFlow version. Please see requirements.txt")
...
logger = utils.CustomLogger(conf.log_path, "IHM", model_name)

# look embedding lookup table
...
# initialize tensors for labels (Y), and time-series ICU measurements (X)
...
""" Important!
Word embeddings are large. In general, if you have 4gb of GPU memory or less, you will not
be able to fit both the model and the word embeddings on the GPU. To work around this,
word embeddings can be run on a CPU."""

if bool(args['big_gpu']):
...
if model_name != 'baseline':
    ...
    if model_name == 'avg_we':
        ...
        embeds += utils.positional_encoding(text, T, N,
        ...
        for i in range(conf.num_blocks):
            with tf.variable_scope("num_blocks_{}".format(i)):
                # Multihead Attention
                enc = utils.multihead_attention(queries=enc, keys=embeds, num_units=hidden_units,
                                                num_heads=10, dropout_rate=dropout_keep_prob, causality=False)
                # Feed Forward
        ...
        rnn_outputs_text, rnn_outputs_text_last = tf.nn.dynamic_rnn(rnn_cell_text, embeds,
                                                                    dtype=tf.float32,
                                                                    sequence_length=text_lens)
        rnn_outputs_text_last = tf.nn.dropout(rnn_outputs_text_last,
        ...
        if GRU:
            ...
            rnn_outputs_text, rnn_outputs_text_last = tf.nn.dynamic_rnn(rnn_cell_text, text_embeddings_pooled,
                                                                        dtype=tf.float32, sequence_length=text_lens_div)
        else:
            rnn_cell_text = rnn.LSTMCell(num_units=rnn_nodes, name='lstm_text')
            _, rnn_outputs_text_last = tf.nn.dynamic_rnn(rnn_cell_text, text_embeddings_pooled,
                                                         dtype=tf.float32, sequence_length=text_lens_div)
            rnn_outputs_text_last = rnn_outputs_text_last.h
        rnn_outputs_text_last = tf.nn.dropout(rnn_outputs_text_last, keep_prob=dropout)  # dropout_keep_probability
        ...
        if pool:
            ...
            text_embeddings = tf.layers.conv2d(inputs=tf.expand_dims(text_embeddings, -1),
                                               filters=filters_2d, kernel_size=(nodes, 1),
                                               strides=(nodes,1), padding='same',
                                               activation='relu', name='Text_Conv_2D',
                                               kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=0.01))
...
if args['split_loss']:
    ...
    loss_full = loss * conf.alpha + loss_t * (1-conf.alpha)
    probs_full = probs * conf.alpha + probs_t * (1-conf.alpha)
...
# select metrics for evaluating model training
with tf.name_scope('train_metric'):
    aucroc, update_aucroc_op = tf.metrics.auc(labels=y, predictions=probs)
    aucpr, update_aucpr_op = tf.metrics.auc(labels=y, predictions=probs, curve="PR")
...
if not(args['TEST_MODEL']):
    ...
    normalizer_state = conf.norm_state
    ...
    eval_reader = InHospitalMortalityReader(dataset_dir=os.path.join(conf.ihm_path, 'train'),
                                            listfile=os.path.join(conf.ihm_path, 'val_listfile.csv'),
                                            period_length=48.0)
...
if args['mode'] == 'test':
    test_reader = InHospitalMortalityReader(dataset_dir=os.path.join(conf.ihm_path, 'test'),
                                            listfile=os.path.join(conf.ihm_path, 'test_listfile.csv'),
                                            period_length=48.0)

    test_raw = ihm_utils.load_data(test_reader, discretizer, normalizer, conf.small_part, return_names=True)
    text_reader_test = utils.TextReader(conf.test_textdata_fixed, conf.test_starttime_path)
    test_text = text_reader_test.read_all_text_concat_json(test_raw['names'], 48)
...
def validate(data_X_val, data_y_val, data_text_val, batch_size, word2index_lookup,
    ...
    if summ_writer is not None:
        ...
        with open(logger.res_filename, 'a') as handle:
    ...
    if (epoch + 1) % 20 == 0 and save:
        save_path = saver.save(sess, os.path.join(logger.folder, 'ckpt_e{}'.format(epoch)))
...
with tf.Session(config=gpu_config) as sess:
    ...
    summ_writer = tf.summary.FileWriter(logger.folder, sess.graph)
    ...
    for epoch in range(number_epoch):
        ...
        last_best_val_aucpr, changed = validate(eval_data_X, eval_data_y, eval_data_text,
                                                ...
                                                True, epoch=epoch, summ_writer=summ_writer)
        if changed:
            early_stopping += 1
        ...
        if early_stopping >= 15:
            tf.logging.info("AUCPR didn't change from last 15 epochs, early stopping")
            break
